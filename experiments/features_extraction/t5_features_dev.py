...
from core.models.modules.heads import T5ClassificationHead
...
def get_features(data_loader,mymodel,device):
    ...
    with torch.no_grad():
        for index, batch in enumerate(tqdm(data_loader)):
            ...
            features.append(outputs[0][:,-1,:].cpu().numpy())
            ids.append(myid)
...
pickle.dump(res_dict, open("./features_dev/t5_features.pkl", "wb"))
